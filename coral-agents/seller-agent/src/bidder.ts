...
export function sellerConfigFromEnv(name: string): SellerConfig {
  return {
    name,
    services: (process.env.SERVICES ?? 'copyrescue').split(',').map((s) => s.trim()).filter(Boolean),
    floorSol: Number(process.env.FLOOR_SOL ?? '0.0003'),
    persona: process.env.PERSONA ?? 'a landing page conversion copy specialist selling concise copy rescues',
  }
}
