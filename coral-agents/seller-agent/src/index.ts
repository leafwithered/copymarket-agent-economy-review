/**
 * CopyMarket seller agent for the CoralOS market.
 *
 * Flow:
 ...
 * seller verifies the funded escrow using the vault PDA before delivering a copy rescue.
 */
...
function boundReference(order: Quote & { round: number }): string {
  const preimage = `copymarket-coral:${order.round}:${order.service}:${order.arg}:${SELLER_WALLET}:${order.priceSol}`
