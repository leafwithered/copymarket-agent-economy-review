...
The CopyMarket buyer broadcasts a `WANT`, collects competing seller bids, awards the best value, and
settles the winner through arbiter-gated escrow by default. It buys a compact landing-page copy rescue:
hero rewrite, CTA, section copy, conversion notes, and risk controls.
...
`SETTLEMENT_MODE=direct` keeps the legacy base escrow path available, but the CopyMarket round uses
...
`SELLER_WALLET` binds the payout wallet. `BUYER_SERVICE` defaults to `copyrescue`, and
`BUYER_ARG` defaults to a crypto-wallet landing-page clarity request. `MARKET_SELLERS` controls the
competing sellers.
...
Live settlement signs devnet transactions and is exercised through the CoralOS market runner.
