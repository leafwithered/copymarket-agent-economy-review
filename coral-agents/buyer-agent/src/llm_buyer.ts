 ...
 * that decides to pay."
 *
 * Lives in buyer-agent as a legacy pay-per-call helper. It uses Anthropic's HTTP API directly so the
 * buyer package stays light and can be tested without pulling an SDK.
 *
...
async function createAnthropicMessage(input: {
  ...
  if (!key) throw new Error('ANTHROPIC_API_KEY is required for LLMBuyerStrategy')
...
export class LLMBuyerStrategy {
  ...
  async purchase(log: ActionLog = logToConsole): Promise<string> {
    ...
    const messages: AnthropicMessage[] = [{ role: 'user', content: this.config.goal }]
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      const toolUses = resp.content.filter((c): c is ToolUseBlock => c.type === 'tool_use')
